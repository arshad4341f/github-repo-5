...
async function executeSandwichTrade(tx, preTxPrice, postTxPrice) {
  // Calculate potential profit for front-run trade
  const amount = 10; // Example amount to trade
  ...
  // Add your front-run trade logic here
  ...
  // Add your back-run trade logic here
}
...
async function viewBalanceInUSDT() {
  ...
  const usdtContract = new web3.eth.Contract(USDT_ABI, USDT_ADDRESS); // Define USDT_ABI and USDT_ADDRESS
  const usdtDecimals = await usdtContract.methods.decimals().call();
  const usdtPrice = await fetchPriceFromGraphAPI('uniswapV2', USDT_ADDRESS, 'ETH'); // Example DEX and ETH as the second token
